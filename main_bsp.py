...
def start_game():
    ...
    HIGHSCORE_FILE = os.path.join(BASE, "highscore.json")  #wichtig
    ...
    while running:
        ...
        hud = FONT.render(
            f"Score: {current_score}  Highscore: {highscore}  Lives: {player.lives}  Armor: {player.armor}  Fragments: {stats['fragments']}  Wave: {wave+1}",
            True,(255,255,255))
        ...
        if player.lives <= 0:
            ...
            #falls der Score höher als der bisherige ist, dann nach den Namen fragen und speichern
            if final_score > highscore:
                player_name = ask_player_name()
                save_highscore(final_score, player_name)
